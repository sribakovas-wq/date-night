...
let i=0,drawing=false,revealed=false,moves=0,changing=false;
...
function paintCover(){
 ...
 g.addColorStop(0,'#efa1b8');
 g.addColorStop(.5,'#ce6888');
 g.addColorStop(1,'#87384f');
...
function scratch(e){
 ...
 ctx.beginPath();
 ctx.arc(x,y,Math.max(22,canvas.width*.06),0,Math.PI*2);
 ctx.fill();
...
/* VIENAS PRISILIETIMAS / PASPAUDIMAS = VIENAS PERĖJIMAS */
function goNext(e){
 if(e){
  e.preventDefault();
  e.stopPropagation();
 ...
 if(i<slides.length-1){
  ...
  document.querySelector('#story').innerHTML=`
   <section class="final">
    ...
    <p>
     Brangioji, būsiu pasiruošęs.<br><br>
     Šįkart planas tavo,<br>
     o aš su malonumu leisiuosi nustebinamas. ❤️
    </p>
...
/* pointerup telefone ir kompiuteryje suveikia nuo vieno prisilietimo */
next.addEventListener('pointerup',goNext);
